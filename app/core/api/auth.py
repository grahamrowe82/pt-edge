...
TIER_LIMITS = settings.RATE_LIMIT_TIERS
...
async def optional_api_key(request: Request) -> dict:
    ...
    _prefix = settings.API_KEY_PREFIX
    _key_len = settings.API_KEY_LENGTH
    ...
    if not raw_key.startswith(_prefix) or len(raw_key) != _key_len:
        raise HTTPException(status_code=401, detail={"error": {"code": "unauthorized", "message": f"Invalid API key format. Keys start with {_prefix} and are {_key_len} characters."}})
...
def validate_api_key(raw_key: str) -> dict | None:
    """Validate an API key without HTTP context. Returns key_data dict or None.
    ...
    _prefix = settings.API_KEY_PREFIX
    _key_len = settings.API_KEY_LENGTH
    if not raw_key or not raw_key.startswith(_prefix) or len(raw_key) != _key_len:
        return None
    key_hash = _hash_key(raw_key)
    key_data = _lookup_key(key_hash)
    if not key_data:
