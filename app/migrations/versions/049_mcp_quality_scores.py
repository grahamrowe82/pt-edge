"""Add MCP server quality scores materialized view

Revision ID: 049
Revises: 048
Create Date: 2026-03-27

Adds mv_mcp_quality: a 0-100 composite quality score for all MCP-domain repos.
Components: maintenance (0-25), adoption (0-25), maturity (0-25), community (0-25).
Includes quality_tier classification and risk_flags array.
"""
...
def upgrade() -> None:
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_mcp_quality CASCADE")

    op.execute("""
CREATE MATERIALIZED VIEW mv_mcp_quality AS
WITH
dep_counts AS (
    SELECT
        ar.id AS repo_id,
...
FROM scored
    ...
    op.execute("CREATE UNIQUE INDEX idx_mv_mcp_quality_id ON mv_mcp_quality (id)")
