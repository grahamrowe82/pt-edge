"""Enhance allocation engine with Bayesian surprise + position/CTR signals

Revision ID: 066
Revises: 065
Create Date: 2026-03-31

Adds three new signals to mv_allocation_scores:
  - surprise_ratio: Bayesian surprise (actual vs expected impression share)
  - position_strength: Google's position = competitive density signal (ES)
  - ctr_vs_benchmark: actual CTR / expected CTR for position (EHS)

Also adds domain_impressions_7d, category_impressions_7d for confidence.
Updates EHS and ES formulas. Adds new columns to snapshots table.
"""
...
# Domain mapping CASE expression — reused for both GSC CTEs
_DOMAIN_CASE = """
        CASE
...
# Subcategory extraction for category pages
...
-- ============================================================
-- GSC: map ALL page URLs to domains (not just category pages)
-- This captures impressions from server detail pages too
-- ============================================================
...
-- Bayesian prior: expected impression share per domain
domain_prior AS (
    SELECT domain,
...
-- Bayesian surprise: actual vs expected share
domain_surprise AS (
    SELECT
        p.domain,
...
-- GSC category-level stats (current week) — for category pages only
gsc_current AS (
    SELECT
        {_DOMAIN_CASE} AS domain,
...
-- Position strength: convert avg position to 0-100 score
-- Position 1 = 95, Position 3 = 85, Position 10 = 50, Position 20+ = 0
gsc_position AS (
...
-- Includes CTR vs benchmark as presentation quality signal
ehs_scored AS (
    SELECT *,
        LEAST(100, ROUND(
...
-- Includes position strength as Google's competitive density signal
scored AS (
    SELECT *,
        LEAST(100, ROUND(
            25 * PERCENT_RANK() OVER (ORDER BY github_star_velocity_7d)
          + 20 * PERCENT_RANK() OVER (ORDER BY github_new_repos_7d)
          + 10 * PERCENT_RANK() OVER (ORDER BY github_fork_acceleration_7d)
          + 25 * (1 - gsc_coverage_ratio)
          + 20 * PERCENT_RANK() OVER (ORDER BY COALESCE(position_strength, 0))
        ))::int AS es
    FROM ehs_scored
)
...
def upgrade() -> None:
    # 1. Drop and recreate MV with enhanced scoring
    ...
    op.execute("""
        ...
        ADD COLUMN IF NOT EXISTS surprise_ratio NUMERIC(8,4),
        ADD COLUMN IF NOT EXISTS position_strength NUMERIC(6,2),
        ADD COLUMN IF NOT EXISTS ctr_vs_benchmark NUMERIC(8,4),
        ADD COLUMN IF NOT EXISTS domain_impressions_7d INT
    """)

    # 3. Recreate the deep dive queue view
    op.execute("""
...
def downgrade() -> None:
    ...
    op.execute("""
        ALTER TABLE allocation_score_snapshots
        DROP COLUMN IF EXISTS surprise_ratio,
        DROP COLUMN IF EXISTS position_strength,
        DROP COLUMN IF EXISTS ctr_vs_benchmark,
        DROP COLUMN IF EXISTS domain_impressions_7d
    """)
    # Note: does NOT recreate the 065 version — run 065 upgrade for that
