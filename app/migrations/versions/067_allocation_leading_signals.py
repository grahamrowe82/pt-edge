"""Add leading signals to allocation engine + content budget infrastructure

Revision ID: 067
Revises: 066
Create Date: 2026-03-31

Adds HN velocity, newsletter mentions, release activity, and content coverage
gaps to mv_allocation_scores. Creates content_budget table for allocation-driven
content generation and repo_briefs table for decoupled brief generation.

ES formula updated to include leading demand signals (HN, newsletters, releases).
EHS formula unchanged.
"""
...
# Domain mapping CASE expression — reused for GSC CTEs
_DOMAIN_CASE = """
        CASE
...
-- ============================================================
-- LEADING DEMAND SIGNALS (NEW in 067)
-- ============================================================

-- HN velocity: posts + points in last 7d per (domain, subcategory)
hn_velocity AS (
    SELECT ar.domain, ar.subcategory,
...
-- Newsletter mention density in last 7d per (domain, subcategory)
...
-- Release activity in last 7d per (domain, subcategory)
release_velocity AS (
    SELECT ar.domain, ar.subcategory,
...
-- Content coverage: fraction of repos with AI summaries per category
content_coverage AS (
...
-- ============================================================
-- GSC SIGNALS (unchanged from 066)
-- ============================================================
gsc_all_by_domain AS (
    SELECT
        {_DOMAIN_CASE} AS domain,
...
domain_prior AS (
    SELECT domain,
           COUNT(*)::numeric / NULLIF(SUM(COUNT(*)) OVER (), 0) AS expected_share
    FROM ai_repos
    WHERE domain <> 'uncategorized'
    GROUP BY domain
),
...
domain_surprise AS (
    SELECT
        p.domain,
        COALESCE(a.impressions, 0) AS domain_impressions_7d,
        CASE WHEN SUM(COALESCE(a.impressions, 0)) OVER () > 0
            THEN (COALESCE(a.impressions, 0)::numeric / SUM(COALESCE(a.impressions, 0)) OVER ())
                 / NULLIF(p.expected_share, 0)
            ELSE 0
        END AS surprise_ratio
    FROM domain_prior p
    LEFT JOIN gsc_all_by_domain a ON p.domain = a.domain
),

gsc_current AS (
    SELECT
        {_DOMAIN_CASE} AS domain,
        {_SUBCAT_EXTRACT} AS subcategory,
        SUM(impressions) AS impressions,
        SUM(clicks) AS clicks,
    ...
    GROUP BY 1, 2
    HAVING {_SUBCAT_EXTRACT} IS NOT NULL
),
gsc_previous AS (
    SELECT
        {_DOMAIN_CASE} AS domain,
...
gsc_stats AS (
    SELECT
        ...
        CASE WHEN gp.avg_position IS NOT NULL AND gc.avg_position IS NOT NULL
            THEN gp.avg_position - gc.avg_position
            ELSE NULL
...
gsc_position AS (
    SELECT domain, subcategory,
           ...
           CASE WHEN gsc_avg_position IS NOT NULL
               THEN GREATEST(0, 100 - LEAST(gsc_avg_position, 20) * 5)
               ELSE NULL
           ...
           CASE WHEN gsc_impressions_7d > 0 AND gsc_avg_position IS NOT NULL
               THEN (gsc_clicks_7d::numeric / gsc_impressions_7d)
                    / CASE
                        WHEN gsc_avg_position <= 1 THEN 0.398
                        WHEN gsc_avg_position <= 2 THEN 0.187
                        WHEN gsc_avg_position <= 3 THEN 0.102
                        WHEN gsc_avg_position <= 4 THEN 0.072
                        WHEN gsc_avg_position <= 5 THEN 0.051
                        WHEN gsc_avg_position <= 6 THEN 0.044
                        WHEN gsc_avg_position <= 7 THEN 0.030
                        WHEN gsc_avg_position <= 8 THEN 0.021
                        WHEN gsc_avg_position <= 9 THEN 0.019
                        WHEN gsc_avg_position <= 10 THEN 0.016
                        ELSE 0.010
                    END
               ELSE NULL
           END AS ctr_vs_benchmark
    FROM gsc_stats
),
...
-- EHS: Established Heat Score (0-100) — UNCHANGED from 066
ehs_scored AS (
    SELECT *,
        LEAST(100, ROUND(
...
-- ES: Emergence Score (0-100) — UPDATED in 067 with leading signals
scored AS (
    SELECT *,
        LEAST(100, ROUND(
...
SELECT
    ...
    newsletter_mentions_7d,
    releases_7d,
    -- Content coverage
    summary_ratio,
    -- Bayesian surprise
    surprise_ratio,
    domain_impressions_7d,
    -- Confidence level
    CASE
...
def upgrade() -> None:
    # 1. Drop and recreate MV with leading signals
    ...
    # 2. Add new columns to snapshot table
    op.execute("""
        ALTER TABLE allocation_score_snapshots
        ADD COLUMN IF NOT EXISTS hn_posts_7d INT,
        ADD COLUMN IF NOT EXISTS hn_points_7d INT,
        ADD COLUMN IF NOT EXISTS newsletter_mentions_7d INT,
        ADD COLUMN IF NOT EXISTS releases_7d INT,
        ADD COLUMN IF NOT EXISTS summary_ratio NUMERIC(5,4)
    """)

    # 3. Create content_budget table (transient, TRUNCATE + INSERT per run)
    op.execute("""
    ...
    """)
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_content_budget_pipeline "
        "ON content_budget(pipeline)"
    )

    # 4. Create repo_briefs table (decoupled from projects)
    op.execute("""
        CREATE TABLE IF NOT EXISTS repo_briefs (
            id SERIAL PRIMARY KEY,
            ai_repo_id INT NOT NULL REFERENCES ai_repos(id) ON DELETE CASCADE,
    ...
    # 5. Recreate the deep dive queue view
    op.execute("""
...
def downgrade() -> None:
    op.execute("DROP TABLE IF EXISTS repo_briefs CASCADE")
    op.execute("DROP TABLE IF EXISTS content_budget CASCADE")
    op.execute("DROP VIEW IF EXISTS v_deep_dive_queue CASCADE")
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_allocation_scores CASCADE")
    op.execute("""
        ALTER TABLE allocation_score_snapshots
        DROP COLUMN IF EXISTS hn_posts_7d,
        DROP COLUMN IF EXISTS hn_points_7d,
        DROP COLUMN IF EXISTS newsletter_mentions_7d,
        DROP COLUMN IF EXISTS releases_7d,
        DROP COLUMN IF EXISTS summary_ratio
    """)
    # Note: does NOT recreate 066 version — run 066 upgrade for that
