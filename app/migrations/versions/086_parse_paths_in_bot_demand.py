"""Parse paths once in mv_access_bot_demand, eliminate LIKE joins

Revision ID: 086
Revises: 085
Create Date: 2026-04-06

The LIKE join `path LIKE '%/servers/' || full_name || '/%'` caused a
40-minute query that pegged production CPU. It's O(n*m) with no index
help — scanning 475K MV rows against 220K ai_repos rows.

Fix: parse path structure once in mv_access_bot_demand (page_type,
domain, full_name, subcategory_slug), add index on ai_repos.full_name,
and rewrite all downstream joins as indexed equality joins.

Affects: mv_access_bot_demand, mv_allocation_scores, mv_owner_demand,
v_deep_dive_queue.
"""
...
def upgrade() -> None:
    # --- 0. Index on ai_repos.full_name for equality joins ---
    op.execute("CREATE INDEX ix_ai_repos_full_name ON ai_repos (full_name)")

    # --- 1. Recreate mv_access_bot_demand with parsed path columns ---
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_owner_demand CASCADE")
    ...
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_access_bot_demand CASCADE")
    ...
    op.execute("""
        ...
        WITH classified AS (
            SELECT
                ...
                CASE
                    -- Tier 1: AI user-action crawlers
        ...
        SELECT
            ...
            -- Parsed path columns: extract once, join cheaply everywhere
            CASE
                WHEN path LIKE '%%/servers/%%' THEN 'server'
                WHEN path LIKE '%%/categories/%%' THEN 'category'
                WHEN path LIKE '%%/compare/%%' THEN 'compare'
                ELSE 'other'
            END AS page_type,
            SPLIT_PART(path, '/', 2) AS domain,
            CASE WHEN path LIKE '%%/servers/%%'
                 THEN SPLIT_PART(path, '/', 4) || '/' || SPLIT_PART(path, '/', 5)
            END AS full_name,
            CASE WHEN path LIKE '%%/categories/%%'
                 THEN SPLIT_PART(path, '/', 4)
    ...
    """)
    # Index for downstream joins on full_name
    op.execute("""
        CREATE INDEX ix_mv_access_bot_demand_fullname
            ON mv_access_bot_demand (full_name)
            WHERE page_type = 'server'
    """)

    # --- 2. Recreate mv_allocation_scores with equality joins ---
...
-- Tier 1 AI browsing: equality join via pre-parsed full_name column
...
ai_browsing_category AS (
    SELECT bad.domain,
           bad.subcategory_slug AS subcategory,
           SUM(hits) AS hits,
    ...
    WHERE bad.bot_family IN ({tier1_list})
      ...
      AND bad.access_date >= CURRENT_DATE - 7
      AND bad.subcategory_slug IS NOT NULL
    GROUP BY bad.domain, bad.subcategory_slug
),
ai_browsing AS (
...
gsc_all_by_domain AS (
    SELECT CASE
            ...
            WHEN page LIKE '%%/perception/%%' THEN 'perception'
            ELSE 'mcp'
        END AS domain,
...
gsc_current AS (
    ...
    GROUP BY 1, 2 HAVING REGEXP_REPLACE(
            REGEXP_REPLACE(page, '.*/categories/([^/]+)/?$', '\\1'),
            '^https://.*$', NULL
        ) IS NOT NULL
),
...
FROM scored
    ...
    # --- 3. Recreate mv_owner_demand with equality join ---
    op.execute(f"""
...
def downgrade() -> None:
    ...
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_allocation_scores CASCADE")
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_access_bot_demand CASCADE")
    op.execute("DROP INDEX IF EXISTS ix_ai_repos_full_name")
    # Would need to recreate previous versions — not implemented
