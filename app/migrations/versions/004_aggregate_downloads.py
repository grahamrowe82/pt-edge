"""Aggregate downloads across sources (PyPI + npm + HuggingFace)

All materialized views previously used DISTINCT ON (project_id) for downloads,
which arbitrarily picked one source per project. This migration replaces those
CTEs with SUM aggregation across sources so that e.g. Whisper gets
PyPI 1.7M + HuggingFace 5.8M = 7.5M total.
...
# Reusable CTE fragment: latest downloads aggregated across all sources
LATEST_DL_CTE = """
latest_dl AS (
...
def upgrade() -> None:
    ...
    # --- 1. mv_momentum with aggregated downloads ---
    op.execute("""
...
dl_prev_7d AS (
    SELECT project_id, SUM(downloads_monthly) AS downloads_monthly
    FROM (
        SELECT DISTINCT ON (ds.project_id, ds.source)
            ds.project_id, ds.source, ds.downloads_monthly
        FROM download_snapshots ds
        JOIN (
            SELECT DISTINCT ON (project_id, source)
                project_id, source, snapshot_date
            FROM download_snapshots
            ORDER BY project_id, source, snapshot_date DESC
        ) l ON ds.project_id = l.project_id AND ds.source = l.source
        WHERE ds.snapshot_date <= l.snapshot_date - 7
        ORDER BY ds.project_id, ds.source, ds.snapshot_date DESC
    ) per_source
    GROUP BY project_id
),
dl_prev_30d AS (
...
WHERE p.is_active = true
    ...
    # --- 2. mv_hype_ratio with aggregated downloads ---
    op.execute(f"""
...
WHERE p.is_active = true
    ...
    # --- 4. mv_lifecycle with aggregated downloads ---
    op.execute(f"""
...
WHERE p.is_active = true
    ...
    # --- 5. mv_project_summary with aggregated downloads ---
    op.execute(f"""
...
def downgrade() -> None:
    # This migration only changes view definitions.
    # Downgrade by dropping and letting 003's upgrade recreate them.
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_project_summary CASCADE")
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_lifecycle CASCADE")
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_project_tier CASCADE")
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_hype_ratio CASCADE")
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_momentum CASCADE")
    # Note: after downgrade, run 003 upgrade + 002 upgrade to restore old views.
