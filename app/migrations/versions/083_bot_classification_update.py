"""Update bot classification: add missing bot families

Revision ID: 083
Revises: 082
Create Date: 2026-04-06

Fixes 6,437 misclassified requests/day:
- GoogleOther (5,872 hits) — Google AI training crawler, was falling to 'human'
- Claude-User (8 hits) — Anthropic user-action, was falling to 'human'
- DuckAssistBot (3 hits) — DuckDuckGo AI, was unclassified
- MJ12bot (530 hits) — Majestic SEO, was falling to 'other_bot'
- PetalBot (31 hits) — Huawei search, was falling to 'other_bot'
- AdsBot-Google (3 hits) — Google Ads, was falling to 'other_bot'
- Qwantbot (1 hit) — Qwant search, was falling to 'other_bot'

Also updates mv_allocation_scores Tier 1 list to include
DuckAssistBot and Claude-User as AI user-action signals.
"""
...
def upgrade() -> None:
    # --- 1. Recreate mv_access_bot_demand with updated CASE ---
    ...
    op.execute("""
        ...
        WITH classified AS (
            SELECT
                ...
                CASE
                    -- Tier 1: AI user-action crawlers (demand = human asked AI)
    ...
    # --- 2. Recreate mv_allocation_scores with updated Tier 1 list ---
...
def downgrade() -> None:
    ...
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_allocation_scores CASCADE")
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_access_bot_demand CASCADE")
    # Would need to recreate 075 + 078 versions — not implemented
