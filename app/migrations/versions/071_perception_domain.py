"""Add perception domain to site infrastructure

Revision ID: 071
Revises: 070
Create Date: 2026-04-01

Creates mv_perception_quality materialized view and rebuilds
mv_allocation_scores with perception in the GSC domain CASE statement.
"""
...
# Quality MV template — same as migration 070 (with created_at-aware ages CTE)
...
# Domain CASE for GSC URL mapping — now includes perception
_DOMAIN_CASE = """
        CASE
...
def upgrade() -> None:
    # 1. Create mv_perception_quality
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_perception_quality CASCADE")
    op.execute(_QUALITY_VIEW_SQL.substitute(
        view_name="mv_perception_quality", domain="perception"
    ))
    op.execute("CREATE UNIQUE INDEX idx_mv_perception_quality_id ON mv_perception_quality (id)")

    # 2. Rebuild mv_allocation_scores with perception in DOMAIN_CASE
...
def downgrade() -> None:
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_perception_quality CASCADE")
    # Note: does NOT rebuild mv_allocation_scores without perception — run 070 for that
