"""Wire patch availability dimension into scoring views.

Revision ID: 006
Revises: 005
Create Date: 2026-04-11

Adds has_fix and fix_versions columns to cves table. Drops and recreates
all materialized views with the patch_availability dimension active:
has_fix=true → patch score 10 (low risk), else 25 (max risk).
"""
...
def upgrade():
    # Add fix columns to cves table
    op.execute("ALTER TABLE cves ADD COLUMN IF NOT EXISTS has_fix BOOLEAN DEFAULT false")
    op.execute("ALTER TABLE cves ADD COLUMN IF NOT EXISTS fix_versions JSONB")
    op.execute("CREATE INDEX IF NOT EXISTS ix_cves_has_fix ON cves (has_fix)")
    ...
    # CVE scores with all 4 dimensions active
    op.execute("""
        ...
        SELECT
            ...
            LEAST(100, (
                ...
                + LEAST(25, (LEAST(15, (LN(COALESCE(c.epss_score, 0) * 100 + 1) * 3.5)::int)
                    + CASE WHEN c.is_kev THEN 5 ELSE 0 END)::int)
    ...
    # Software scores (now includes patch_availability via max of CVE patch scores)
    op.execute("""
        ...
        SELECT s.id, s.cpe_id, s.name,
            ...
            LEAST(100, LEAST(25, COALESCE(agg.max_severity, 0))
                + LEAST(25, COALESCE(agg.max_exploitability, 0))
                + LEAST(25, (LN(GREATEST(1, COALESCE(agg.cve_count, 0)) + 1) * 4)::int)
                + COALESCE(agg.max_patch, 0)
            ) AS composite_score,
            CASE WHEN LEAST(100, LEAST(25, COALESCE(agg.max_severity, 0))
                + LEAST(25, COALESCE(agg.max_exploitability, 0))
                + LEAST(25, (LN(GREATEST(1, COALESCE(agg.cve_count, 0)) + 1) * 4)::int)
        ...
        FROM software s LEFT JOIN (
            SELECT cs.software_id, COUNT(*) AS cve_count,
                   MAX(cv.severity) AS max_severity, MAX(cv.exploitability) AS max_exploitability,
                   MAX(cv.patch_availability) AS max_patch
            FROM cve_software cs JOIN mv_cve_scores cv ON cv.id = cs.cve_id
    ...
    op.execute("""
        ...
        FROM vendors v LEFT JOIN (
            SELECT cv.vendor_id, COUNT(DISTINCT cv.cve_id) AS cve_count,
                   MAX(cs.severity) AS max_severity, MAX(cs.exploitability) AS max_exploitability,
                   MAX(cs.patch_availability) AS max_patch
            FROM cve_vendors cv JOIN mv_cve_scores cs ON cs.id = cv.cve_id
            GROUP BY cv.vendor_id
        ) agg ON agg.vendor_id = v.id
    """)
    op.execute("CREATE UNIQUE INDEX ON mv_vendor_scores (id)")

    # Weakness, technique, pattern scores (unchanged structure, just recreated)
    op.execute("""
        CREATE MATERIALIZED VIEW mv_weakness_scores AS
        SELECT w.id, w.cwe_id, w.name,
            LEAST(25, COALESCE(agg.max_severity, 0)) AS severity,
            LEAST(25, COALESCE(agg.max_exploitability, 0)) AS exploitability,
            LEAST(25, (LN(GREATEST(1, COALESCE(agg.cve_count, 0)) + 1) * 3)::int) AS exposure,
            COALESCE(agg.max_patch, 0) AS patch_availability,
        ...
        FROM weaknesses w LEFT JOIN (
            SELECT cw.weakness_id, COUNT(DISTINCT cw.cve_id) AS cve_count,
                   MAX(cs.severity) AS max_severity, MAX(cs.exploitability) AS max_exploitability,
                   MAX(cs.patch_availability) AS max_patch
            FROM cve_weaknesses cw JOIN mv_cve_scores cs ON cs.id = cw.cve_id
    ...
    op.execute("""
        ...
        SELECT ap.id, ap.capec_id, ap.name,
            LEAST(25, COALESCE(agg.max_severity, 0)) AS severity,
            LEAST(25, COALESCE(agg.max_exploitability, 0)) AS exploitability,
    ...
    op.execute("""
        ...
        SELECT t.id, t.technique_id, t.name,
            LEAST(25, COALESCE(agg.max_severity, 0)) AS severity,
            LEAST(25, COALESCE(agg.max_exploitability, 0)) AS exploitability,
    ...
    # Entity summary with all 6 types
    op.execute("""
...
def downgrade():
    ...
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_software_scores")
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_cve_scores")
    op.execute("DROP INDEX IF EXISTS ix_cves_has_fix")
    op.execute("ALTER TABLE cves DROP COLUMN IF EXISTS fix_versions")
    op.execute("ALTER TABLE cves DROP COLUMN IF EXISTS has_fix")
